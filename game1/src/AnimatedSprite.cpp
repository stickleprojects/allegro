...
bool AnimatedSprite::Draw()
{
    ...
    if (image)
    {
        ...
        if (scale == 1.0f)
        {
            if (color_set)
            {
                al_draw_tinted_bitmap_region(image, color, currentFrame->Region.left, currentFrame->Region.top, currentFrame->Region.width,
                                             currentFrame->Region.height, X, Y, 0);
            }
            else
            {
                al_draw_bitmap_region(image, currentFrame->Region.left, currentFrame->Region.top, currentFrame->Region.width,
                                      currentFrame->Region.height, X, Y, 0);
            }
        ...
        else
        {
            if (color_set)
            {
                al_draw_tinted_scaled_bitmap(image, color,  currentFrame->Region.left, currentFrame->Region.top, currentFrame->Region.width,
                                      currentFrame->Region.height, X, Y, this->Width * this->scale, this->Height * this->scale, 0);
            }
            else
            {
                al_draw_scaled_bitmap(image, currentFrame->Region.left, currentFrame->Region.top, currentFrame->Region.width,
                                      currentFrame->Region.height, X, Y, this->Width * this->scale, this->Height * this->scale, 0);
            }
        }
    }
